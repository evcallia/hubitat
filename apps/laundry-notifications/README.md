...
5. Configure each child instance with the following inputs:
   ...
   - **Vibration sensor:** Choose the vibration/acceleration sensor attached to the
     appliance.
   - **Seconds of continuous activity:** Number of seconds of continuous vibration before 
     a load is considered started.
   ...
   - **Hub variable name:** Select the string hub variable created in step 3 from the
     dropdown list.
   - **Notification devices:** Select one or more notification-capable devices.
   - **Values for each device:** Provide the hub variable value (name) that should trigger each
     device.
...
- Update the hub variable (using dashboards, rules, or other automations) to the name of the
  person responsible for the next load before the cycle finishes. I use a NFC tag by the washer/dryer
  that we scan when starting a load.
